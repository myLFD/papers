...
由上，为了实现对SurfaceFlinger的支持，需要实现OpengGL ES、完成硬件抽象层中的Gralloc和HWC服务。 
...
loongson硬件支持（紫色标注部分）表示现有的龙芯显卡实现相关的软件，需要部分适配，需实现部分（红色标注）表示需要实现的模块。
...
并对整体结构进行了描述。随后对于其中两个主要模块，图形缓存分配模块和硬件混合渲染模块进行需求分析。
