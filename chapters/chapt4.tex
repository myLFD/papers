...
\subsection{瓦片访存优化}
LG110是Tile-Based架构的GPU，支持将几何数据转换成小矩形区域，目前已支持4✖4以及8x8的Tile格式,如表\ref{tab:颜色缓冲区流水线配置}。
Tile-Based Rendering就是可以尽量减少片元着色期间的GPU的外部内存访问量，如图\ref{fig:不同GPU架构数据流向},Immediate架构在执行片元着色时会多次写入主存导致访存压力，
而Tile-based架构的GPU会在tile内一次性光栅化所有的三角形。将片元结果缓存在片上Tile缓冲区，等Tile渲染完再写回主存。

\begin{figure}[h]
  \centering
  \includegraphics[width=0.8\textwidth]{不同GPU架构数据流向.pdf}
  \caption{不同GPU架构数据流向}
...
在安卓图形架构中，使用的是直接渲染框架，因此对Tile-based渲染需要用户态驱动Mesa共同完成支持即可。
