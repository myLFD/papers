...
如图\ref{fig:零面积图元交换缓冲区时间}，本文实现的驱动层效率与AMD R5 520接近。
...
\section{本章小结}
本章对本文测试平台进行了全面的介绍，并详细介绍了测试过程以及调试方法。
接着对所研究和构建的系统进行了从内核到整体的测试。简要介绍了整个测试过程，包括测试流程以及调试方法。在服务器上使用AOSP构建了完整的安卓内核、显卡核模块以及安卓系统镜像，在目标龙芯平台上
