...
与分辨率、着色器复杂度相关；DC60每帧允许的绘制调用次数，衡量 CPU/GPU 提交绘制命令的效率。如图\ref{fig:perf测试结果图}和图\ref{fig:perf测试结果图2}。
其中Solid color和Solid gradient由于是纯色填充，所以不需要对纹理进行采样，因此仅测试纹理类型2。纹理类型1的复杂度要高于纹理类型2,因此1的执行效率要低于2。

\begin{figure}[h]
  \centering
  \includegraphics[width=0.6\textwidth]{gl_perf测试结果图.png}
