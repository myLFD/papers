...
在linux内核中还定义了几个辅助宏用于生成具体的命令码，如代码块\ref{lst:drm-ioctl}。
...
\begin{lstlisting}[label={lst:drm-ioctl}]
...
\begin{table}[h]  
  ...
  \begin{tabular}{llll}
    ...
    命令序号  ​	​ &​ 功能领域​	& 权限位 & 映射函数 \\
    \midrule
    0x00		& ​GEM 内存管理​ & 0x100001 & gsgpu\_gem\_create\_ioctl \\
    0x01		& ​内存映射​ & 0x100001 & gsgpu\_gem\_mmap\_ioctl \\
    0x02		& ​上下文管理​	& 0x100001 & gsgpu\_ctx\_ioctl\\
    0x03		& ​缓冲区对象列表​	& 0x100001 & gsgpu\_bo\_list\_ioctl\\
    0x04		& ​命令流控制​	& 0x100001 & gsgpu\_cs\_ioctl\\
    0x05		& ​设备信息查询​	& 0x100001 & gsgpu\_info\_ioctl\\
    0x06		& ​元数据管理​	& 0x100001 & gsgpu\_gem\_metadata\_ioctl\\
    0x07		& ​同步与闲置状态​	& 0x100001 & gsgpu\_gem\_wait\_idle\_ioctl\\
    0x08		& ​虚拟地址分配​	& 0x100001 & gsgpu\_gem\_va\_ioctl\\
    0x09		& ​命令流同步​	& 0x100001 & gsgpu\_cs\_wait\_ioctl\\
    0x10		& ​通用 GEM 操作​	& 0x100001 & gsgpu\_gem\_op\_ioctl\\
    0x11		& ​用户指针支持​	& 0x100001 & gsgpu\_gem\_userptr\_ioctl\\
    0x12		& ​栅栏同步​	& 0x100001 & gsgpu\_cs\_wait\_fences\_ioctl\\
    0x13		& ​虚拟内存管理​	& 0x100001 & gsgpu\_vm\_ioctl\\
    0x14		& ​栅栏与句柄映射​	& 0x100001 & gsgpu\_cs\_fence\_to\_handle\_ioctl\\
    0x15		& ​任务调度 & 0x000010 & gsgpu\_sched\_ioctl\\
    0x16		& ​硬件信号量操作​	& 0x100001 & gsgpu\_hw\_sema\_op\_ioctl\\
    \bottomrule
    \end{tabular}
...
\begin{enumerate}[label=\arabic*.]
  \item 在内核中为GPU设备分配并初始化一个GEM对象。
  \item 将GEM对象映射到用户空间，允许用户态直接访问显存。
  \item 创建/销毁GPU上下文，管理上下文资源。
  \item 管理一组缓冲区对象操作的集合，用于批量操作。
  \item 提交命令流到GPU硬件执行。
  \item 查询GPU硬件参数
  \item 设置/获取GEM对象的元数据，主要用于tiling模式或者缓存策略的选择。
  \item 等待GEM对象关联的所有GPU操作完成。
  \item 管理GEM对象的虚拟地址绑定，控制GPU虚拟地址空间映射。
  \item 等待指定命令流完成执行。
  \item 对GEM对象执行通用操作，包括创建信息、设置内存放置域等。
  \item 通过用户态指针注册内存作为GEM对象，支持零拷贝数据传输。
  \item 等待多个栅栏信号完成。
  \item 虚拟化管理的基础接口。
  \item 将栅栏转换为句柄，支持跨进程同步。
  \item 配置调度策略，更改命令流执行顺序。
  \item 操作硬件信号量，控制硬件级资源争用。
\end{enumerate}
...
这个宏的作用是将驱动私有的 IOCTL 命令注册到 DRM 的统一命令表中，并关联对应的处理函数和权限标志。如代码块\ref{lst:drm-ioctl-drv}。
...
\begin{lstlisting}[label={lst:drm-ioctl-drv}, escapechar=@]
...
由于drm模块的接口变化为某个函数的增加或删除，部分结构体的属性变化，以及函数的参数变化。以空参数调用某个函数时，若函数相关实现已被删除，则测试代码不会产生编译错误，
仅会在链接时产生错误，而函数存在时，则会产生编译错误，将此种情况命名为function；若某个函数的形参不存在或者结构体或者结构体的某个属性不存在时，则测试代码会产生编译错误，
...
1.某函数实现删除或增加，假设测试函数为funcTest，目标测试函数为funcB，使用一个空返回值空形参的测试函数funcTest并调用无参数的目标函数，
依据目标测试函数funcB的返回值确定测试函数funcTest的返回值，如算法\ref{algo:algorithm2}所示，其中TypeFuncB表示funcB的返回值。

2.某函数实现的形参删除或增加，沿用1假设，添加调用funcB所需的参数，调用funcB并返回，如算法\ref{algo:algorithm3}所示，Type1表示待测试形参的数据类型，arg1表示待测试形参。

3.某结构体的删除或增加，假设目标结构体类型为A，在funcTest中使用A作为形参即可，如算法\ref{algo:algorithm4}所示。
...
\begin{minipage}{0.45\textwidth}
  \begin{algorithm}[H]
    ...
    \Fn{funcTest}{
        \KwResult{TypefuncB}            % 声明返回值类型
        \Return{funcB()}                  % 使用\Return命令
    }
...
\begin{minipage}{0.45\textwidth}
  \begin{algorithm}[H]
    \SetAlgoLined
    \KwData{Type1 arg1,Type2 arg2,...}
    \SetKwProg{Fn}{Function}{:}{}  % 定义函数结构
    \Fn{funcTest}{
        \KwResult{TypefuncB}            % 声明返回值类型
        \Return{funcB(arg1,arg2,...)}                  % 使用\Return命令
    }
...
\begin{minipage}{0.45\textwidth}
  \begin{algorithm}[H]
    ...
    \SetKwProg{Fn}{Function}{:}{}  % 定义函数结构
    \Fn{funcTest}{
        \KwResult{int}            % 声明返回值类型
        \Return{offsetof(struct A,c)}                  % 使用\Return命令
    }
    \caption{测试代码示例4}
...
该功能主要是DRM管理模块完成，在创建hwcomposer回调时，首先完成对DRM设备资源的初始化，需要先打开/dev/dri/card*设备文件，
...
而在模块中，在进行硬件资源初始化之前需要检查对象的属性是否有无异常，其需要先使用drmModeObjectGetProperties通过对象ID和类型获取属性列表指针 props，
同时遍历props通过 drmModeGetProperty获取每个属性的详情。随后如果找到与prop\_name匹配的属性名称，则调用init方法将属性ID、指针、以及值等目标属性存储至私有的std::vector中。
在获取属性详情后使用drmModeFreeProperty释放资源，遍历结束后释放 props。如果找到目标属性就返回0，否则返回-ENOENT。

还有一个重要功能是创建显示资源管线，在DRM资源初始化时会对所有的连接器、编码器和显示控制器等组合的正确性进行验证，随后将正确的组合创建显示资源管线。首先会创建一个DrmDisplayPipeline类型的
std::make\_unique对象来保证唯一性，随后将该管线与连接器、编码器和显示控制器等进行绑定尝试，随后找出设备下的所有plane图层，并需要判断是否主图层是唯一的情况，除此之外为0或者大于1的情况都
报错处理，最后将主图层与管线进行绑定。
...
\subsection{合成计划创建}
该功能会先创建一个空的DrmKmsPlan对象，同时调用pipe.GetUsablePlanes获取当前管线中可分配的平面资源，并逐图层分配平面：遍历上层传入的的图层列表中的图层，
从显示资源管线中支持的平面资源中挑选一个可用平面，如果当前平面不适配该图层则跳过直到找到适配的，如果找不到可用平面，则直接返回空指针，表示计划失败。
最后成功绑定后，创建一个LayerToPlaneJoinin 结构记录layer和plane的绑定，并设置其在z坐标上的叠加层级。所有图层绑定成功后返回最终的DrmKmsPlan对象。

\subsection{验证与显示}
实际上，hwcomposer的客户端调用是在SurfaceFlinger中，通过显示钩子函数实现对服务端具体功能的调用。
由于龙芯目前的硬件不支持硬件混合渲染，所以SurfaceFlinger的画面合成过程实际上是基于OpenGL ES完成的，也就是client模式合成，仅使用了HWComposer1.0接口标准的软件实现
...
像素操作数等统计信息并返回结果。在CreateComposition时，会初始化渲染管道pipe，按照zpos排列图层并导入帧缓冲区，为图层创建DrmKms计划，并进行画面更新。

验证图层首先需要获取按z-order排序的图层列表。检查是否进入Flattening模式，如果图层数量小于等与1则将所有图层标记为 Client 合成，否则计算调用GetClientLayers计算哪些图层无法使用硬件合成，
从而确定客户端合成的起点和大小。如果不是全部Client合成，需要做一次合成创建CreateComposition来测试当前配置是否可行，如果提交失败则回退成Client合成。统计输出最终需要Client合成的图层数并
计算合成像素数和总像素数，并返回通知SurfaceFlinger合成策略是否有变化，返回 HWC2::Error::HasChanges 表示发生了合成策略变化，否则返回 None。 
...
\subsection{缓冲区信息适配}
缓冲区信息适配利用了Gralloc的接口以及安卓HAL的接口。在提取元数据时使用Gralloc的perform函数接口实现对句柄相应属性的提取，如width、height、hal\_format、usage等属性，并将属性填充至
缓冲区对象bo，从而实现对Gralloc缓冲区信息的对接。而在模块导入时，会使用hw\_get\_module加载硬件模块以使用Gralloc模块的接口。
...
并且初始化XDMA引擎，首先是硬件兼容性检查，通过GSGPU\_CTX命令创建硬件上下文，隔离XDMA操作的执行环境，以支持多任务并行提交；同时创建
...
同时使用GSGPU\_GEM\_CLOSE关闭命令缓冲区的GEM对象句柄，并释放XDMA硬件上下文​。随后是DRI驱动资源释放，需要销毁渲染上下文和屏幕对象，并卸载DRI驱动库，并
...
独立处理GPU加速的内存传输操作，利用了LG110硬件XDMA的特性\cite{胡庆海2016,张凯2015}，实现从一个源内存地址到目标内存地址的数据搬运，支持Linear和Tiled 数据格式存储。
首先需要校验参数与准备资源，计算能处理的最大命令数量（不能超过最大单次传输尺寸）。其次将源缓冲区和目标缓冲区分别通过GSGPU\_GEM\_VA的VA\_OP\_MAP映射到GPU的虚拟空间，以便于从GPU中获取这两个地址，
但是在目标缓冲区上添加WRITEABLE标志表示这是写入的标志。随后构造XDMA命令如表\ref{tab:XDMA描述符}所示，按顺序构造完命令后循环构造DMA命令。
...
\textbf{缓冲区创建与销毁}
在使用DRI框架进行缓冲区对象创建时，有使用DRI驱动和构造命令调用内核两种方式。因此在创建缓冲区时，如果Tiling类型为TILE\_TYPE\_DRI，则使用DRI驱动创建缓冲区，在此基础上可以通过判断是否是带有修饰符，
如果带有修饰符，则使用DRI驱动框架中的createImageWithModifiers方法创建缓冲区。createImageWithModifiers 是一个用于创建支持特定内存布局修饰符的图像或缓冲区对象的函数。
而如果元数据中不使用修饰符创建缓冲区则使用createImage创建缓冲区，作用与上一个函数类似。这两种方法在创建时都需要传入宽、高、格式（修饰符），而格式需要从drm格式转换为dri\_image格式，
在此不过多赘述。而第三种是使用内核创建缓冲区，需要先初始化缓冲区布局，为图形缓冲区对象计算和设置与格式相关的属性，如步幅（stride）、大小（size）、偏移量(offset)。
步幅是指从缓冲区中某一行的开始位置到下一行开始位置的字节数。换句话说，它是缓冲区内每行数据的跨度，提供了当前行跳到下一行的信息。
偏移量是指从缓冲区的起始位置到某个数据元素的内存地址的距离。大小表示缓冲区所占用的内存空间的大小，通常以字节为单位。
内存分配参数选择GTT内存域，并执行GSGPU\_GEM\_CREATE内核调用，创建GTT内存，如图\ref{fig:lgbm后端-3-1}。
...
\subsection{HAL接口功能实现模块}
%缓冲区同步，缓冲区复用
从安卓系统服务跨进程调用缓冲区相关实现，具体来说就是使用系统的handle句柄，向Gralloc服务申请相关的缓冲区。而安卓系统服务往往会申请多个缓冲区
或同时对同一个缓冲区进行多次申请，因此实现缓冲区快速申请是必经之路。因此设计如图\ref{fig:handle-buffer存储哈希表}，
本地存储缓冲区和句柄的结构是两个哈希表，一个用于对gralloc\_handle\_t和gralloc\_imported\_handle\_info进行哈希映射，
info包含缓冲区和该句柄引用该缓冲区的数量，用于支持对已注册缓冲区句柄的多次访问支持。另一个对缓冲区id和缓冲区进行哈希映射以实现快速访问。
...
而驱动的allocate的算法设计如算法\ref{algo:GRALLOC_DRIVER::ALLOCATE}，主要是使用上一节所述的支持龙芯的Gralloc后端方法完成缓冲区的创建，获取宽、高、格式、步长、压缩标志等信息用于
...
最后描述了系统构建，包括构建安卓OpenGL ES支持、安卓系统镜像定制以及Tile访存优化。
